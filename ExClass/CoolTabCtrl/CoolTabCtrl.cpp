...
static CPen _penB(PS_SOLID,1 ,RGB(200,200,200));//Tab阴影
...
void CCoolTabCtrl::OnPaint() 
{
	...
	for(pos=m_PageList.GetHeadPosition();pos!=NULL;nItemIndex++)
	{
		...
		if(pItem)
		{
			if (m_nActivePage == nItemIndex && m_nActivePage != 0)//设置选中的tab颜色
...
CCoolTabCtrl::CPageItem* CCoolTabCtrl::AddPage(CRuntimeClass* pClass,UINT nIDTemplate, LPCTSTR sText, UINT IconID)
{
	CCoolTabCtrl::CPageItem *pItem;
	CDialog *pDlg = (CDialog*)pClass->CreateObject();
	if(pDlg != NULL)
	{
		if(pDlg->Create(nIDTemplate,this))
		{
			pItem = AddPage(pDlg, sText, IconID);
			if(pItem)
			{
...
CCoolTabCtrl::CPageItem* CCoolTabCtrl::AddPage(CRuntimeClass* pClass,UINT nIDTemplate,LPCTSTR sText,HICON hIcon)
{
	CCoolTabCtrl::CPageItem *pItem;
	CDialog *pDlg = (CDialog*)pClass->CreateObject();
	if(pDlg != NULL)
	{
		if(pDlg->Create(nIDTemplate,this))
		{
			pItem = AddPage(pDlg, sText, hIcon);
			if(pItem)
			{
				pItem->SetPageStyle(1);
				return pItem;
...
CCoolTabCtrl::CPageItem* CCoolTabCtrl::AddPage(CWnd *pWnd, LPCTSTR sText, HICON hIcon)
{
	ASSERT(pWnd);
	ASSERT(IsWindow(pWnd->m_hWnd));
...
void CCoolTabCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	for(pos=m_PageList.GetHeadPosition();pos!=NULL;nItemIndex++)
	{
		...
		if(pItem)
		{
			...
			{
				if (m_nActivePage == nItemIndex)		//点击的是当前tab则不更新
					break;
				pItem->m_pWnd->PostMessage(WM_SIZE);	//不加这个点击tab之后不更新界面大小
...
extern BOOL	m_bShutdown;
void CCoolTabCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	UINT nItemIndex=0;
	POSITION pos;
	CPageItem *pItem;
	for(pos = m_PageList.GetHeadPosition(); pos != NULL; nItemIndex++)
	{
		...
		if(pItem)
		{
			if(pItem->m_rect.PtInRect(point))
			{
				...
				if (pItem->m_pWnd)
				{
					if (MessageBox("确认要关闭该窗口吗?", "提示", MB_YESNO | MB_ICONQUESTION) == IDNO)
						return;
					pItem->m_pWnd->ShowWindow(SW_HIDE);
					m_PageList.RemoveAt(m_PageList.FindIndex(nItemIndex));
					SetActivePage(nItemIndex - 1);
					break;
				}
			}
		}
